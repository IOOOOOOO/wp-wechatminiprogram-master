...
//绘制圆形图片
function circleImg(context,imgUrl, x, y, r) {
  context.save();
  var d = 2 * r;
  var cx = x + r;
  var cy = y + r;
  ...
  context.clip();
  context.drawImage(imgUrl, x, y, d, d);
  context.restore();
}
...
function drawTitleExcerpt(context, title, excerpt) {
    ...
    if (getStrLength(title) <= 14) {
        ...
        context.fillText(title.substring(0, 15), 75, 450);
        context.fillText(title.substring(15, 30), 75, 500);
    }
    context.setFontSize(24);
    context.setTextAlign('left');
    context.setGlobalAlpha(0.7);    
    for (var i = 0; i <= 46; i += 16) {
        //摘要只绘制前50个字，这里是用截取字符串
        if (getStrLength(excerpt)>46)
        {
            if ( i == 32) {
                context.fillText(excerpt.substring(i, i + 16) + "...", 140, 570 + i * 2);
            }
            else {
                context.fillText(excerpt.substring(i, i + 16), 140, 570 + i * 2);
            }
